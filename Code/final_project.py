    ...
    if Data != [] :
        ...
        if len(Data[details[0]][1]) == 0:
            ...
            if now == False:
                ...
                if (p, q) not in Data[details[0]][1]:
                    for i in range(len(Data[details[0]][1])):
                        if p[0] < Data[details[0]][1][i][0][0]:
                            Data[details[0]][1].insert(i, (p,q))
                            break
                            
                        elif i == len(Data[details[0]][1])-1 and p != Data[details[0]][1][i][0] and q!= Data[details[0]][1][i][1]:
                            Data[details[0]][1].insert(i, (p,q)) 
...
def generatebill(G, di, details, pds):
    ...
    for i in di:
        ...
        for j in range(len(receipt)):
            #print(receipt[j]) 
            if receipt[j] in G[i][1] and receipt[j] not in ch:
    ...
    for k in dis:
        #print(k)
        message0 = str(k[0])+ ' prepare order for ' + details[0] + ' Number = ' + details[2] + " Address: " + v.get() + ' email : ' + details[3]+ ". "
        #print(k[1][0])
        #print(k[1][0][0][0])
        #print(k[1][0][1])
    ...
    for i in range(len(receipt)):
        ...
        message += (str(receipt[i][0]) + " "*(23-len(receipt[i][0])) + str(receipt[i][1])+ " "*(26-len(receipt[i][1])) + str(z[i])+"\n")
